...
또한 **next sentence prediction** task를  사전학습 과정에서 같이 사용합니다.
...
`BERT_base`의 경우, 비교를 위해 `GPT` 모델과 같은 크기로 설정되었는데, 차이점을 다시 한번 언급하면
...
이러한 문제를 해결하기 위해, 입력 token의 일부를 무작위로 mask 시킨 다음, [mask]token을 예측합니다.
